...
│
├── 03_cal_mae/                        # 関節角度 MAE 計算スクリプト
│
├── 04_mae_heatmap/                    # MAE ヒートマップ・集計
│
├── 05_max_angle_error/                # 最大角度誤差
│
├── 06_direction_detection/            # 方向角誤差・相関分析（最重要）
│   ├── process_all_data.py
│   ├── scripts/
...
### 方向角誤差（06_direction_detection と照合済み）
...
詳細は `06_direction_detection/output/correlation_analysis/` の CSV を読むこと。
...
- **座標系の統一手順**（4ページ版では簡略）を詳しく説明
  ...
  - Y反転の具体的な変換式
  - `06_direction_detection/scripts/coordinate_transform.py` を参照して実装詳細を記述
- 可視度フィルタ（0.5未満除外）の影響
...
- 現行の Table 1 に加えて、Y レンジ別（Y=0.5,1.5 vs Y=1.0,2.0）の比較も追加
- カメラ高さによる誤差変化の考察

**5.2 Direction Angle Error（∆θ, ∆ψ）**
- 全関節の ∆θ・∆ψ 統計をまとめた **新しい Table** を追加（4ページ版にない）
  - `06_direction_detection/output/processed_data/joint_summary.csv` から取得
...
```
これで `06_direction_detection/output/` に CSV が生成される。

### 既存データを直接読む場合
CSV ファイルを直接 Read ツールで読み込んで数値を確認すること。
- 方向角誤差サマリー: `06_direction_detection/output/processed_data/joint_summary.csv`
- 相関行列: `06_direction_detection/output/correlation_analysis/correlation_matrix_theta.csv`
- 詳細データ: `06_direction_detection/output/processed_data/detailed_results.csv`
...
### Y軸反転の適用
`06_direction_detection` の全出力は Y軸反転適用済み。実装は `06_direction_detection/scripts/coordinate_transform.py` を参照。
...
5. `06_direction_detection/output/processed_data/joint_summary.csv` を読んで数値を確認
6. `06_direction_detection/output/correlation_analysis/` の CSV を読んで相関値を確認
