...
\WarningFilter{lastpage}{Rerun to get the references right}
...
体系的に特定・定量化する．
得られた知見は，これら\textbf{バイアスと推定器の限界を理解したうえで}用いる
\textbf{補助的・研究用の評価指標}を支えることを意図し，
歩行者検知や衝突回避など安全クリティカルな主系統（検出，測距，時間的余裕の推定等）を
直接置き換えるものではない．
...
\textbf{リアルタイム}に近い低遅延推定が特に求められるのは，
スポーツ中継やトレーニングのオンライン解析，工場・倉庫での安全監視，
リハビリテーション現場での運動観察，XRやゲーム内アバタ制御など，
逐次映像から即応する必要がある応用である．
Googleが開発したMediaPipeは，GPUを必要とせず
コモディティハードウェア上でリアルタイム処理を実現できる点で
実務上の採用が急速に拡大している\cite{mediapipe}．
一方，監視カメラや車載のように視点・照明・遮蔽が一定でない場面では，
全身姿勢推定は物体検出や測距，衝突までの時間的猶予の推定など\textbf{主系統を代替する指標になりにくく}，
\textbf{バイアスと限界を押さえたうえでの補助情報や，研究・開発時の評価軸}として位置づけるのが現実的である．
本研究はその前提を定量的に与えることも目的とする．
...
実施されていない．
シミュレーションベースのGT収集は，SURREAL\cite{surreal}が先駆けとなり，
Unityベースのジェネレータ\cite{peoplesanspeople}などに発展し，
...
COCO\cite{coco}をはじめとする一般物体・人物写真ベースの公開データでは，
インスタンスごとに多様なポーズは含まれるものの，
立位や歩行に近いポーズ，および正面に近いカメラ視点が相対的に多いと
しばしば議論されており，
これが学習モデルに正面視点バイアスを埋め込み，
側方・俯瞰カメラへの汎化性能を低下させうる．
...
本手法はBazarevskyら\cite{blazepose}が提案した．
COCOバリデーションセットおよび独自ベンチマークで
高い精度が報告されている．
...
COCO\cite{coco}をはじめとする一般公開コーパスでは，
立位・歩行に近いポーズや正面近傍の視点が相対的に多い傾向が指摘され，
学習モデルに正面視点バイアスを埋め込みうる．
この仮説のもと，側方・俯瞰配置のカメラでは精度が低下しうるが，
本研究の多層評価がその傾向を定量的に確認する（表~\ref{tab:mae_by_layer}）．
...
\caption{2種の評価指標の\emph{簡略化した}概念図（幾何関係の説明用）．
  左：関節角度（MAE）；3点による屈曲角（$0$--$180^{\circ}$）．
  右：方向角（$\Delta\theta$，$\Delta\psi$）；股関節中心を基準とした角度オフセット（$-180^{\circ}$から$+180^{\circ}$）．}
...
符号が全関節で反転する．
そこでMediaPipe出力の$y$成分の符号を反転し（実装上は$y\leftarrow -y$），Unityの上向き$Y$と揃えた．
処理は\path{05_direction_detection/scripts/coordinate_transform.py}に実装した．
以下で報告する全ての方向角結果はこの座標統一後の値を用いる．
...
関節角度MAEは\path{03_joint_angle_mae}内の\path{Y=0.5/}，\path{Y=1.0/}，\path{Y=1.5/}，\path{Y=2.0/}各フォルダの\path{coordinate_angle_mae.csv}を結合して集計し，
方向角・相関は\path{05_direction_detection/output/processed_data}と
\path{05_direction_detection/output/correlation_analysis}配下
...
段階 & パス \\
\midrule
Y軸統一 & \path{05_direction_detection/scripts/coordinate_transform.py} \\
関節角度MAE & \path{03_joint_angle_mae/Y=0.5/}〜\path{Y=2.0/}の各\path{coordinate_angle_mae.csv}（4層結合） \\
方向角統計 & \path{05_direction_detection/output/processed_data/joint_summary.csv} \\
股関節詳細 & \path{05_direction_detection/output/processed_data/detailed_results.csv} \\
相関表 & \path{05_direction_detection/output/correlation_analysis/*.csv} \\
カメラ良否 $|\Delta\theta|$ & \path{05_direction_detection/output/processed_data/frame_camera_summary.csv} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{公開リポジトリ．}
Unityによる撮影環境は\url{https://github.com/MASHMARO2718/bias_evaluation_mediapipe_unity_enenvironment}，
本稿の数値・図表の再現に用いた解析スクリプトは\url{https://github.com/MASHMARO2718/bias_evalutation_mediapipe_unity_ieej2026}で公開する（表~\ref{tab:repro_pipeline}の相対パスは後者リポジトリのルート基準）．
MediaPipe処理済みの\path{CapturedFrames_*.csv}（\path{02_mediapipe_processed/}に展開して配置）の公開先は文献\cite{zenodo_mp_csv}．

\subsubsection{統計解析}

表~\ref{tab:correlation_theta}，\ref{tab:correlation_psi}のPearson相関係数$r$は，
同一フレーム・同一カメラ上で得た2関節の誤差（例：$\Delta\psi_{\mathrm{L}}$と$\Delta\psi_{\mathrm{R}}$）を
行としてプールしたとき，その\textbf{対応のある}2列$x_i,\,y_i$（$i=1,\ldots,n$）について
\textbf{線形}な共変動の強さを示す（因果や非線形依存は表さない）．
定義は
\[
  r=\frac{\sum_{i=1}^{n}(x_i-\bar{x})(y_i-\bar{y})}
         {\sqrt{\sum_{i=1}^{n}(x_i-\bar{x})^2}\,\sqrt{\sum_{i=1}^{n}(y_i-\bar{y})^2}}
\]
で，$-1\le r\le 1$．$n$は上記プールの行数（各関節ペアで$n>21{,}000$）である．
$H_0:\rho=0$に対する両側検定はキャプションに$p<0.001$と記した．

表~\ref{tab:direction_angle_error}の\textbf{SD}は，\emph{1関節・1種類の誤差}（例：$|\Delta\theta|$）について，
プールした$n$観測$e_1,\ldots,e_n$の\textbf{標本標準偏差}
$\mathrm{SD}=\sqrt{\frac{1}{n-1}\sum_{i=1}^{n}(e_i-\bar{e})^2}$であり，
「誤差が平均からどれだけ散らばるか」を表す（平均・SDは関節ごとに独立，2関節間の連動は$r$で示す）．

一方，母相関$\rho$の近似95\%信頼区間はFisherの$z$変換$z=\mathrm{arctanh}(r)$に対し，
$\mathrm{SE}(z)\approx 1/\sqrt{n-3}$として区間を構成する（大標本で$z$の分布が平均$\mathrm{arctanh}(\rho)$，分散$1/(n-3)$の正規分布で近似できることに基づく）．
この\textbf{SE}（標準誤差）は「標本の$r$から推定した母相関$\rho$の不確かさ」の目安であり，
誤差そのもののばらつきを表すSDとは意味が異なる．
$n$が大きいため$|r|\approx 0.8$付近でも区間幅は0.02未満に収まる（表~\ref{tab:bias_claim_evidence}）．
...
\caption{関節角度誤差統計（関節角度MAE，単位：度）．
  出典：\texttt{coordinate\_angle\_mae.csv}（\texttt{03\_joint\_angle\_mae}配下，4層結合）．}
...
\begin{figure*}[!tb]
  ...
  \caption{左肘の関節角度MAEヒートマップ（カメラ高さ3層）．
    ...
    出典：\texttt{03\_joint\_angle\_mae/}（層別\texttt{coordinate\_angle\_mae.csv}）．}
...
\begin{figure}[!t]
  ...
  \caption{カメラ高さ層別（Y$=$0.5, 1.0, 1.5, 2.0\,m）の関節角度MAE．
    ...
    出典：\texttt{03\_joint\_angle\_mae/}．}
...
\caption{カメラ高さ層別の関節角度MAE（単位：度）．
  出典：\texttt{03\_joint\_angle\_mae}配下の層別\texttt{coordinate\_angle\_mae.csv}．}
...
\caption{Y軸座標統一の効果：$y \leftarrow -y$適用前後の平均$|\Delta\theta|$（単位：度）．
  出典：\texttt{coordinate\_transform.py}適用前後の比較（リポジトリ\texttt{05\_direction\_detection}；3.4節）．}
...
本推定器は各ランドマークをヒートマップと回帰で推定する\cite{blazepose}．
出力は股関節中心を基準とした3次元座標である．
このアーキテクチャは骨長一貫性や骨盤剛性などの
解剖学的拘束を陽的に組み込んでいない．
座標統一後も肘$|\Delta\theta|\approx58^{\circ}$の残差が持続し，
方向バイアスが根強く残ることを示す．

COCO\cite{coco}などの学習コーパスは，
日常的シーンで立位や歩行に近いポーズ，正面近傍の視点が相対的に多い傾向があり，
肘方向推定を下垂姿勢（体側）にバイアスさせる可能性が高いと解釈できる．
左右肘誤差の逆符号パターン（$r=-0.862$）は
左右フリップのデータ拡張と\emph{整合する}（MediaPipeの学習詳細は非公開）．
肘$\Delta\theta$のSD（関節ごとのプール観測にわたるばらつき）が$\approx23$--$25^{\circ}$と小さいことは，
系統誤差がノイズに埋もれにくく，比較的予測可能で後処理補正の余地があることを示す．
...
Unityシミュレーションと505多視点カメラにより，MediaPipeの系統的バイアスを定量化した．
関節MAEと層別視点依存は表~\ref{tab:joint_angle_error}，\ref{tab:mae_by_layer}，
$y\leftarrow -y$の効果は表~\ref{tab:yflip_effect}，
股$\Delta\psi$負相関・上肢連動・肘残差の解釈は第5節および表~\ref{tab:correlation_theta}--\ref{tab:correlation_psi}にまとめた．
得られた定量化は，視点偏重や座標慣例など予測可能な誤差構造を，導入設計や補助的推論の前提として利用する際の材料となる．
なお統計は合成GTの再現条件下の挙動を示し，実被験者・野外映像への外挿は別途検証が必要である．

今後は骨盤剛性拘束の導入，多視点データに基づく学習改善，他推定器への手順拡張が課題である．
...
F.~Taira, ``MediaPipe processed pose CSVs,'' Zenodo, Mar.~2026, dataset.
\url{https://doi.org/10.5281/zenodo.19296530}
...
\begin{biography}
\profile{n}{平良 文磨}{沖縄県立開邦高等学校2年に在学．琉球大学主催の琉大SEARCHプログラムに基づく琉大カガク院の二段階生として加藤 司准教授の研究室にて研究に取り組み，モーショントラッキング技術に関する研究に従事している．}
\profile{m}{加藤 司}{1999年3月，東京水産大学（現東京海洋大学）卒業．沖縄県立高等学校に勤務しながら2021年3月，琉球大学大学院総合知能工学科修了．博士（工学）．現在，琉球大学大学院教育学研究科高度教職実践専攻准教授として映像授業の開発及びモバイルラーニングシステムの開発，技術動画の開発及び効果の検証，生成AIの教育利活用に関する研究に従事している．}
\profile{n}{安富祖 仁}{2004年3月，琉球大学工学部情報工学科卒業．2013年3月，同大学大学院博士後期課程理工学研究科総合知能工学専攻修了．博士（工学）．同年4月より北海道大学大学院知識メディアラボラトリに特命研究員として勤務．2015年4月より琉球大学医学部附属病院特任助教．2017年12月より琉球大学工学部工学科社会基盤コース特命研究員を経て，現在はデータサイエンティストとして人工知能研究に従事．}
\end{biography}
