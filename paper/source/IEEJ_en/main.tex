...
\begin{itemize}\itemsep0pt
  \item \textbf{Coordinate system bias}: Systematic error in $\theta$ when
        ...
        $y$-axis; mean elbow $|\Delta\theta|$ is $\approx121^{\circ}$ before
        unification and $\approx58^{\circ}$ after $y \leftarrow -y$, and mean
        shoulder $|\Delta\theta|$ falls from $\approx170^{\circ}$ to
        $\approx10^{\circ}$
...
$\approx121^{\circ}$ for elbows before $y \leftarrow -y$).
...
\caption{Concept diagram of the two evaluation metrics. Left: joint angle
  (MAE) --- flexion angle of three points ($0$--$180^{\circ}$). Right: direction
  angle ($\Delta\theta$, $\Delta\psi$) --- angular offset relative to the
  hip centre ($-180^{\circ}$ to $+180^{\circ}$). The figure is a simplified schematic;
...
($\approx23$--$25^{\circ}$) is small enough that the bias is relatively
