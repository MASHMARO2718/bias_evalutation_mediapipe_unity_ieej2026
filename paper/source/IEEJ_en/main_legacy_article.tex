...
\begin{itemize}\itemsep0pt
  \item \textbf{Coordinate system bias}: A Y-axis sign mismatch between Unity
        and MediaPipe inflates elbow direction error from $\approx121\degree{}$ to
        $58\degree{}$ after correction; shoulder error drops from $\approx170\degree{}$ to
        $\approx10\degree{}$.
  \item \textbf{Pelvic rotation artefact}: Left--right hip depth errors are
        strongly anti-correlated ($r=-0.84$), creating a phantom pelvic
        rotation caused by the absence of pelvis rigidity constraints.
  \item \textbf{Upper-limb coupling bias}: Elbow--shoulder--wrist errors in
        the depth direction are positively correlated ($r=0.72$--$0.77$),
        indicating hierarchical error propagation along the kinematic chain.
  \item \textbf{Viewpoint bias}: Joint angle MAE increases by up to 15\% at
        overhead camera height (Y\,=\,2.0\,m), consistent with the frontal-view
        prior embedded in BlazePose training data.
\end{itemize}
...
inflates direction angle errors by up to $\approx120\degree{}$.

The contributions of this paper are as follows:
\begin{enumerate}\itemsep0pt
  \item \textbf{Bias taxonomy for MediaPipe}: We identify and categorise
        four distinct systematic biases in MediaPipe pose estimation---
        coordinate mismatch, pelvic artefact, upper-limb coupling, and
        viewpoint degradation---each with quantitative characterisation.
  ...
  \item \textbf{Large-scale multi-viewpoint dataset}: 505 camera positions
        ...
        enabling the first systematic viewpoint-bias analysis for MediaPipe.
\end{enumerate}
...
Table~\ref{tab:yflip_effect} quantifies the per-joint improvement.
...
\caption{Effect of Y-axis coordinate unification: mean $|\Delta\theta|$ (deg)
  ...
  Source: \texttt{7\_direction\_ditection} (before) and
  \texttt{11\_direction\_ditection} (after).}
...
The training data for BlazePose (primarily COCO~\cite{coco} and fitness
benchmarks~\cite{blazepose}) is dominated by frontal and near-frontal viewpoints;
