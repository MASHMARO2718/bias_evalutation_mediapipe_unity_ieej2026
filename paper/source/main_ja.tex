...
\usepackage{array}
\usepackage{float}
\usepackage{dblfloatfix}  % 二段組で figure* を下段にも配置可能に（余白穴対策）
% 二段組フロートを詰める（デフォルトは厳しくて空白ページ化しやすい）
...
\maketitle
\begin{center}\small
\textbf{Bias Evaluation of MediaPipe Pose Estimation Using a Unity Simulation Environment}
\end{center}
...
\section*{キーワード}
\noindent\textbf{キーワード：}MediaPipe，BlazePose，姿勢推定，バイアス評価，Unity，シミュレーション，方向角誤差

\noindent\textbf{Keywords:} MediaPipe, BlazePose, pose estimation, bias evaluation, Unity simulation, direction angle error
...
未修正では近位関節の平均$|\Delta\theta|$が例えば肘で$\approx121\degree{}$に達しうる．
...
MediaPipeは\texttt{model\_complexity=1}，
\texttt{static\_image\_mode=True}，
\texttt{min\_detection\_confidence=0.5}で推論し，
可視性スコアが0.5未満のランドマークは解析から除外した．
GT関節座標はUnityの\texttt{HumanBodyBones}からCSV出力した．
関節角度MAEは\texttt{04\_mae\_heatmap/coordinate\_angle\_mae.csv}
（および層別サブフォルダ）から集計し，
方向角・相関は
\texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv}，
\texttt{detailed\_results.csv}，
\texttt{06\_direction\_detection/output/correlation\_analysis/}，
フレーム別最良・最悪カメラは同フォルダの
\texttt{frame\_camera\_summary.csv}から算出した．
...
段階 & パス \\
\midrule
Y軸統一 & \texttt{06\_direction\_detection/scripts/coordinate\_transform.py} \\
関節角度MAE & \texttt{04\_mae\_heatmap/coordinate\_angle\_mae.csv} \\
方向角統計 & \texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv} \\
股関節詳細 & \texttt{06\_direction\_detection/output/processed\_data/detailed\_results.csv} \\
相関表 & \texttt{06\_direction\_detection/output/correlation\_analysis/*.csv} \\
カメラ良否 $|\Delta\theta|$ & \texttt{.../processed\_data/frame\_camera\_summary.csv} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{座標統一後の方向角誤差統計（単位：度，絶対値平均）．
  出典：\texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv}．
...
\begin{figure}[!t]
  ...
  \caption{カメラ高さ層別（Y$=$0.5, 1.0, 1.5, 2.0\,m）の関節角度MAE．
    ...
    出典：\texttt{03\_joint\_angle\_mae/}（層ごとの\path{coordinate_angle_mae.csv}）．}
...
\caption{カメラ高さ層別の関節角度MAE（単位：度）．
  出典：\texttt{03\_joint\_angle\_mae/Y=0.5/}〜\texttt{Y=2.0/}の各\path{coordinate_angle_mae.csv}（4層）．}
...
（フレーム間の最小値$6.1\degree{}$，最大値$15.8\degree{}$）．
\emph{最悪}カメラは平均$80.1\degree{}$（最小$55.6\degree{}$，最大$148.1\degree{}$），
...
\caption{Y軸座標統一の効果：$y \leftarrow -y$適用前後の平均$|\Delta\theta|$（単位：度）．
  出典：\texttt{06\_direction\_detection}の出力（\texttt{coordinate\_transform.py}
  適用前後の比較；3.4節）．}
