...
% ============================================================
%  main.tex
%  Bias Evaluation of MediaPipe Pose Estimation
%  Using a Unity Simulation Environment
%
%  Previous theme: "Accuracy Evaluation" (narrower framing)
%  Current theme : "Bias Evaluation" -- focuses on characterising
%    *systematic*, *predictable* error patterns (viewpoint bias,
%    Y-axis mismatch, pelvic artefact, upper-limb coupling)
%    rather than single-number accuracy reporting.
%
...
%    IEEJ packages: run prepare_ieej_overleaf.py then upload IEEJ_en/ or IEEJ_ja/
%    figs/
...
%      fig08_mae_layer_bar.png
%      bio_taira.jpg  (JPEG; do not use .png extension)
%      bio_kato.jpg
%      bio_afuso.jpg
% ============================================================
...
\usepackage{dblfloatfix}  % allow full-width floats at bottom in twocolumn
...
\maketitle
\begin{center}\small
（和文表題）Unity 環境を用いた MediaPipe 姿勢推定のバイアス評価
\end{center}
...
This study characterises the \emph{systematic biases} in MediaPipe pose
estimation errors using ground truth collected from a Unity simulation
environment with 505 multi-view cameras across four height layers.
The GT is \emph{synthetic} (rendered humanoid, controlled lighting and
geometry); the reported statistics therefore characterise estimator
behaviour under these reproducible conditions, and claims about real-world
human capture require independent validation.
Whereas prior evaluations report single-number accuracy metrics under
limited viewpoint conditions, we focus on identifying \emph{predictable,
structured} error patterns that arise from model architecture, training-data
priors, and coordinate system conventions---biases that can, in principle,
be corrected by post-processing.

We collected 259,356 joint observations (107 frames $\times$ 505 cameras
$\times$ 12 joints) and analysed two complementary error types: joint angle
MAE and direction angle error ($\Delta\theta$, $\Delta\psi$). Four
systematic biases are identified and quantified:
\begin{itemize}\itemsep0pt
  \item \textbf{Coordinate system bias}: Systematic error in $\theta$ when
        Unity's Y-up convention is paired with MediaPipe's image-oriented
        $y$-axis; mean elbow $|\Delta\theta|$ is $\approx121\degree{}$ before
        unification and $\approx58\degree{}$ after $y \leftarrow -y$, and mean
        shoulder $|\Delta\theta|$ falls from $\approx170\degree{}$ to
        $\approx10\degree{}$
        (Table~\ref{tab:yflip_effect}; Sec.~3.4).
  \item \textbf{Pelvic rotation artefact}: Apparent pelvic twist from
        anti-correlated left--right hip depth errors ($\Delta\psi$;
        $r=-0.84$) despite a rigid GT pelvis---a pattern explained by the lack
        of explicit pelvis rigidity in the lifted skeleton
        (Figure~\ref{fig:hip_psi_heatmap}; Table~\ref{tab:correlation_psi}).
  \item \textbf{Upper-limb coupling bias}: Coordinated $\Delta\psi$ (and
        related $\Delta\theta$) among shoulder, elbow, and wrist
        ($r=0.72$--$0.77$), consistent with hierarchical propagation or
        implicit coupling in monocular lifting
        (Tables~\ref{tab:correlation_theta}--\ref{tab:correlation_psi};
        Figure~\ref{fig:correlation_heatmaps}).
  \item \textbf{Viewpoint bias}: Joint angle MAE rises with camera height,
        by up to ${\approx}15\%$ at Y\,=\,2.0\,m vs.\ Y\,=\,1.0\,m in
        Table~\ref{tab:mae_by_layer}, matching the spatial trend in
        Figure~\ref{fig:mae_layer_compare} and consistent with a
        frontal-view prior in training data.
\end{itemize}
...
absent. Simulation-based ground truth collection---pioneered
by SURREAL~\cite{surreal} and later adopted by Unity-based
...
viewpoint distribution~\cite{mehta2017}. Models trained primarily on
near-frontal, standing or walking poses (COCO~\cite{coco} and similar
large-scale benchmarks) embed a strong frontal prior that degrades generalisation
to lateral and overhead cameras. This study directly tests this hypothesis
across 505 camera positions and additionally identifies a systematic
Y-axis coordinate mismatch between Unity and MediaPipe that, if uncorrected,
yields spuriously large mean $|\Delta\theta|$ for proximal joints (e.g.,
$\approx121\degree{}$ for elbows before $y \leftarrow -y$).
...
\begin{enumerate}\itemsep0pt
  ...
  \item \textbf{Coordinate system bias correction}: We identify a
        ...
        quantify its per-joint impact (up to $161\degree{}$ reduction in
  ...
  \item \textbf{Direction angle metrics}: We introduce $\Delta\theta$ and
        ...
        MAE; these metrics reveal structured error patterns---particularly
        the pelvic artefact---that aggregate MAE conceals.
...
Public datasets such as COCO~\cite{coco} are dominated by near-frontal,
standing or walking poses, embedding a strong frontal viewpoint prior in
learned models. This bias causes accuracy to
...
\texttt{model\_complexity=1}, \texttt{static\_image\_mode=True},
\texttt{min\_detection\_confidence=0.5}; landmarks with visibility
$<\!0.5$ were dropped before joint-wise analysis. Ground-truth joint
positions were exported from Unity (\texttt{HumanBodyBones}); the
Y-axis unification for direction angles is implemented in
\texttt{06\_direction\_detection/scripts/coordinate\_transform.py}.
Joint-angle MAE aggregates are produced from
\texttt{04\_mae\_heatmap/coordinate\_angle\_mae.csv} (and layer splits
under \texttt{04\_mae\_heatmap/}); direction-angle summaries and
correlations from
\texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv},
\texttt{detailed\_results.csv}, and
\texttt{06\_direction\_detection/output/correlation\_analysis/}; per-frame
best/worst-camera statistics from
\texttt{frame\_camera\_summary.csv} in the same processed-data folder.
...
\begin{tabular}{ll}
\toprule
Stage & Path (under repository root unless noted) \\
\midrule
Y-axis unification & \texttt{06\_direction\_detection/scripts/coordinate\_transform.py} \\
Joint-angle MAE & \texttt{04\_mae\_heatmap/coordinate\_angle\_mae.csv} \\
Direction-angle stats & \texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv} \\
Hip detailed stats & \texttt{06\_direction\_detection/output/processed\_data/detailed\_results.csv} \\
Correlation tables & \texttt{06\_direction\_detection/output/correlation\_analysis/*.csv} \\
Camera best/worst $|\Delta\theta|$ & \texttt{.../processed\_data/frame\_camera\_summary.csv} \\
\bottomrule
\end{tabular}
\end{table}
...
Pearson correlations in Tables~\ref{tab:correlation_theta}
and~\ref{tab:correlation_psi} are computed over pooled
frame--camera--joint observations ($n>21{,}000$ for each reported pair);
two-sided tests of $H_0:\rho=0$ are summarised as $p<0.001$ in the table
captions. Approximate 95\% confidence intervals for $\rho$ use the
Fisher $z$ transformation,
$z=\mathrm{arctanh}(r)$ with
$\mathrm{SE}(z)\approx 1/\sqrt{n-3}$---because $n$ is large, intervals
are narrow (Table~\ref{tab:bias_claim_evidence}).
For joint-angle MAE across camera height layers
(Table~\ref{tab:mae_by_layer}), we report descriptive means per layer;
formal omnibus tests (e.g., repeated-measures or clustered
non-parametric comparisons across layers) are not central to the
bias-characterisation goal here but should accompany any claim of
``statistically significant layer differences'' in follow-on work.
...
\caption{Direction angle error statistics after coordinate unification
  ...
  \texttt{06\_direction\_detection/output/processed\_data/joint\_summary.csv}.
  $^\ddagger$Hip values computed from \texttt{detailed\_results.csv}
  ($n=21{,}613$ per hip joint) as Hip is excluded from \texttt{joint\_summary.csv}.}
...
\begin{figure}[!t]
  ...
  \caption{Joint angle MAE by camera height layer (Y = 0.5, 1.0, 1.5, 2.0\,m).
    ...
    degradation at the hip (\(+4.7\degree{}\) for right hip vs.\ Y\,=\,1.0).
    Source: \texttt{4\_MAE\_HEATMAP/}.}
...
\caption{Joint angle MAE (deg) by camera height layer.
  Source: \texttt{04\_mae\_heatmap/Y=0.5,1.5/} and
  \texttt{04\_mae\_heatmap/Y=1.0.2.0/coordinate\_angle\_mae.csv}.}
...
\caption{High-correlation joint pairs in XY plane ($\Delta\theta$; $|r|>0.7$).
  All pairs: $p<0.001$ ($n>21{,}000$ frame--camera observations).}
...
Position Error (MPJPE)---the Euclidean distance between estimated and
...
\texttt{06\_direction\_detection/output/processed\_data/frame\_camera\_summary.csv},
107 frames $\times$ 201 cameras per frame).
The \emph{best} camera per frame averages $|\Delta\theta|=8.8\degree{}$ over frames
(per-frame minima and maxima: $6.1\degree{}$ and $15.8\degree{}$).
The \emph{worst} camera averages $80.1\degree{}$ (min $55.6\degree{}$, max $148.1\degree{}$),
...
Shoulder and ankle joints benefit the most ($\Delta|\Delta\theta|>155\degree{}$),
because their ground-truth $\theta$ values are close to $\pm90\degree{}$,
so the sign inversion nearly doubles the apparent error.
Wrist joints show the smallest improvement ($<10\degree{}$) owing to
...
\caption{Effect of Y-axis coordinate unification: mean $|\Delta\theta|$ (deg)
  ...
  Source: \texttt{06\_direction\_detection} exports (before vs.\ after
...
datasets such as COCO~\cite{coco} in which everyday, near-frontal,
standing/walking poses dominate. The arm-at-side
...
to proprietary MediaPipe training details); the standard deviation of elbow $\Delta\theta$
...
recordings~\cite{blazepose}) is dominated by frontal and near-frontal views,
with lateral and top-down cameras underrepresented.
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Single estimator (scope boundary):}
        ...
        (Sec.~3.3--3.4, Table~\ref{tab:repro_pipeline}).
\end{itemize}
...
\begin{itemize}\itemsep2pt
  \item Joint-wise MAE and the Y-layer trend (including overhead degradation)
        are consolidated in Tables~\ref{tab:joint_angle_error}
        and~\ref{tab:mae_by_layer}.
  \item Coordinate unification ($y \leftarrow -y$) substantially reduces
        $|\Delta\theta|$ for several joints; magnitudes appear in
        Table~\ref{tab:yflip_effect}.
  \item Residual elbow $|\Delta\theta|$ is systematic with moderate SD,
        leaving room for post-processing (Section~5).
  \item Hip $|\Delta\psi|$ and its strong negative correlation indicate
        missing pelvic rigidity (Section~5; Table~\ref{tab:correlation_psi}).
  \item Upper-limb error coupling suggests hierarchical propagation
        (Tables~\ref{tab:correlation_theta}--\ref{tab:correlation_psi}).
\end{itemize}
...
\end{thebibliography}
% ============================================================
% Author biographies (after references; one-column for readability)
% ============================================================
\clearpage
\onecolumn
...
\begin{minipage}[t]{1.15in}
  \includegraphics[width=1.1in,keepaspectratio]{figs/bio_taira.jpg}
\end{minipage}\hspace{0.8em}%
\begin{minipage}[t]{\dimexpr\linewidth-1.15in-0.8em\relax}
  \textbf{Fumimaro Taira} (Non-member) is a student at Okinawa Prefectural
  Kaiko High School, Okinawa, Japan. His research interests include
  computer vision, human pose estimation, and quantitative evaluation of
  vision algorithms in simulated environments.
\end{minipage}
...
\noindent
\begin{minipage}[t]{1.15in}
  \includegraphics[width=1.1in,keepaspectratio]{figs/bio_kato.jpg}
\end{minipage}\hspace{0.8em}%
\begin{minipage}[t]{\dimexpr\linewidth-1.15in-0.8em\relax}
  \textbf{Tsukasa Kato} (Member, IEEE) is with the Graduate School of
  Education, University of the Ryukyus, Okinawa, Japan. His research
  interests include educational technology, learning analytics, and
  applications of motion sensing in teaching and learning contexts.
\end{minipage}
...
\noindent
\begin{minipage}[t]{1.15in}
  \includegraphics[width=1.1in,keepaspectratio]{figs/bio_afuso.jpg}
\end{minipage}\hspace{0.8em}%
\begin{minipage}[t]{\dimexpr\linewidth-1.15in-0.8em\relax}
  \textbf{Jin Afuso} (Non-member) is with lollol Inc. His research
  interests include software engineering for interactive systems and
  practical deployment of vision-based human--computer interaction.
\end{minipage}
