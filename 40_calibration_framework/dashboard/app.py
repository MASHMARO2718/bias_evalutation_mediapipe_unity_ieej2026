...
# Error · MC（20_pose_correction/run_error_mc_analysis.py の出力・案 B）
ERROR_MC_DIR = REPO_ROOT / "20_pose_correction" / "error_mc_analysis" / "results"
...
# MA Noise（20_pose_correction/run_ma_noise_rejection.py）
MA_NOISE_DIR = REPO_ROOT / "20_pose_correction" / "ma_noise_rejection" / "results"
...
def build_error_mc_tab():
    ...
    alert = (
        ...
        if has_data else
        "データなし。先に "
        "python 20_pose_correction/run_error_mc_analysis.py を実行してください。"
    )
...
def build_ma_noise_tab():
    ...
    alert = (
        ...
        if has_data else
        "データなし。 python 20_pose_correction/run_ma_noise_rejection.py を実行してください。"
    )
...
def update_emc_graph(camera, joint, metric):
    ...
    if df.empty:
        ...
        fig.update_layout(
            ...
            annotations=[dict(
                text="python 20_pose_correction/run_error_mc_analysis.py",
                xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False,
            )],
        )
