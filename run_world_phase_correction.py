...
やること（既定 = 論文の最終結果だけを再現）:
  ...
  0. world landmarks 抽出   → 2_pose/mediapipe_world_csv/
  1. world + 位相索引の補正 → 2_pose/world_landmark_model/   ★最終形
...
REQUIRED_INPUTS = [
    (ROOT / "1_input" / CALIB_CAM / "video.mp4", "較正動画"),
    (ROOT / "1_input" / CALIB_CAM / "gt_joints.csv", "較正 GT"),
    (ROOT / "1_input" / "aditional__test_data" / VAL_CAM / "video.mp4",
     "検証動画"),
    (ROOT / "1_input" / "aditional__test_data" / VAL_CAM / "gt_joints.csv",
     "検証 GT"),
...
def check_inputs() -> bool:
    ...
    if not ok:
        ...
        print("  動画・GT が無い場合: 1_input/ の配置を確認してください。")
    return ok
