...
DATA = {
    ...
    "angle_mae": {
        "Y=0.5": REPO_ROOT / "3_joint_angle_mae/joint_angle_mae_csv/Y=0.5/coordinate_angle_mae.csv",
        "Y=1.0": REPO_ROOT / "3_joint_angle_mae/joint_angle_mae_csv/Y=1.0/coordinate_angle_mae.csv",
        "Y=1.5": REPO_ROOT / "3_joint_angle_mae/joint_angle_mae_csv/Y=1.5/coordinate_angle_mae.csv",
        "Y=2.0": REPO_ROOT / "3_joint_angle_mae/joint_angle_mae_csv/Y=2.0/coordinate_angle_mae.csv",
    },
    # 方向角誤差集計: joint × (θ_mean, θ_std, ψ_mean, ψ_std, 3D_error) の全カメラ統合
    # columns: joint, theta_mean, theta_std, ..., psi_abs_mean, error3d_mean, ...
    "joint_summary": REPO_ROOT / "5_direction/output/processed_data/joint_summary.csv",

    # フレーム×カメラ別の方向角統計
    # columns: frame_id, camera, num_joints, mean_abs_delta_theta, mean_abs_delta_psi, ...
    "frame_camera_summary": REPO_ROOT / "5_direction/output/processed_data/frame_camera_summary.csv",

    # 全観測の詳細結果（サイズ大・git 除外）。存在すれば符号付き誤差を利用可能
    # columns: (詳細は 5_direction/README.md 参照)
    "detailed_results": REPO_ROOT / "5_direction/output/processed_data/detailed_results.csv",

    # 相関行列
    "corr_theta": REPO_ROOT / "5_direction/output/correlation_analysis/correlation_matrix_theta.csv",
    "corr_psi":   REPO_ROOT / "5_direction/output/correlation_analysis/correlation_matrix_psi.csv",
}
