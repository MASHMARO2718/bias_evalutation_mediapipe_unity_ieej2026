...
前提:
  ...
  - 1_input/ に video.mp4 + gt_joints.csv
  - 2_pose/ に MediaPipe CSV（未完了なら --step 0 で処理）

流れ（v1 と同様に 03〜07 も含む）:
  0. MediaPipe 動画処理          → 2_pose/
  1. 3点角 MAE（層別）           → 3_joint_angle_mae/joint_angle_mae_csv/Y=*/
  2. MAE 統計テーブル統合
  3. 最大角度誤差（04）          → 4_max_angle_error/calculation/Y=*/
  4. 方向角・相関（05）          → 5_direction/output/
  5. θ検証（06）                 → 6_theta_check/
  6. ダッシュボード（07）        → http://127.0.0.1:8050/
  7. キャリブレーション（09）    → 7_correction/outputs/
...
def backup_v1_outputs() -> None:
    ...
    targets = [
        ROOT / "3_joint_angle_mae" / "joint_angle_mae_csv",
        ROOT / "3_joint_angle_mae" / "coordinate_angle_mae_combined.csv",
        ROOT / "5_direction" / "output",
        ROOT / "7_correction" / "outputs",
    ]
...
def step_angle_mae() -> bool:
    ...
    mp_base = ROOT / "2_pose" / "mediapipe_processed_csv"
    gt_glob = str(ROOT / "1_input" / "CapturedFrames_*" / "gt_joints.csv")
    out_base = ROOT / "3_joint_angle_mae" / "joint_angle_mae_csv"
    script = ROOT / "3_joint_angle_mae" / "coordinate_angle_comparison.py"
...
def step_max_angle() -> bool:
    ...
    mp_base = ROOT / "2_pose" / "mediapipe_processed_csv"
    gt_glob = str(ROOT / "1_input" / "CapturedFrames_*" / "gt_joints.csv")
    script = ROOT / "4_max_angle_error" / "calculate_max_angle_error.py"
    ...
    for y in Y_LAYERS:
        ...
        if n == 0:
            print(f"スキップ（CSVなし）: {mp_dir}")
            continue
        out_dir = ROOT / "4_max_angle_error" / "calculation" / y
        out_dir.mkdir(parents=True, exist_ok=True)
...
def step_theta_verify() -> bool:
    ...
    ok = run_cmd([sys.executable, "run_all.py"], ROOT / "6_theta_check")
    if not ok:
        print("警告: 6_theta_check が失敗（継続）")
...
def count_mp_csv() -> int:
    base = ROOT / "2_pose" / "mediapipe_processed_csv"
