...
プロジェクトルート直下の 50_dashboard/ に配置。
32_direction_detection の output を参照。
"""
...
# パス設定: プロジェクトルートと 32_direction_detection
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))
dir_direction = project_root / "32_direction_detection"
...
# ── 角度時系列データ（40_calibration_framework の一括出力）─────────────
# scripts/output/<v1|v2>/CapturedFrames_X_Y_Z/angle_timeseries_<joint>_<safe>.csv
ANGLE_TS_BASE = project_root / "40_calibration_framework" / "scripts" / "output"
    ...
    if df_ts is None or df_ts.empty:
        fig.update_layout(
            title=f"データなし: {camera} / {joint} ({version})<br>"
                  f"<sub>40_calibration_framework/scripts/batch_angle_timeseries.py で生成してください</sub>"
        )
        return fig
