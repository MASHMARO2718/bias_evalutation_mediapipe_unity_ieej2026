...
`09_calibration_framework` の符号付き補正（Model 4S など）の前提データです。
...
フレームレベルの符号付き方向角誤差。`09_calibration_framework` の符号付き補正に使用。
...
| ファイル | 場所 | 説明 |
|---------|------|------|
| MediaPipe CSV | `02_mediapipe_processed/mediapipe_processed_csv/Y=*/` | フレーム別 3D 関節座標 |
...
- **入力元:** `02_mediapipe_processed/` (MediaPipe CSV), `synced_joint_positions.csv` (GT)
- **出力先:** `09_calibration_framework/` が `detailed_results.csv` を使用
- **ダッシュボード:** `07_dashboard/` の可視化でも参照
