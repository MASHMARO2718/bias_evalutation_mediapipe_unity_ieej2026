...
プロジェクトルート直下の 8_dashboard/ に配置。
5_direction の output を参照。
"""
...
# パス設定: プロジェクトルートと 5_direction
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))
dir_direction = project_root / "5_direction"
...
# ── 角度時系列データ（7_correction の一括出力）─────────────
# scripts/output/<v1|v2>/CapturedFrames_X_Y_Z/angle_timeseries_<joint>_<safe>.csv
ANGLE_TS_BASE = project_root / "7_correction" / "scripts" / "output"
    ...
    if df_ts is None or df_ts.empty:
        fig.update_layout(
            title=f"データなし: {camera} / {joint} ({version})<br>"
                  f"<sub>7_correction/scripts/batch_angle_timeseries.py で生成してください</sub>"
        )
        return fig
