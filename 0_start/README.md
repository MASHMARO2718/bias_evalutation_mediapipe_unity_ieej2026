...
`1_input/CapturedFrames_{X}_{Y}_{Z}/` に次があること:

- `video.mp4`
- `gt_joints.csv`

MediaPipe CSV は `2_pose/mediapipe_processed_csv/Y=*/`（未処理なら次のコマンドに `--with-mediapipe`）。
...
| GUI | コマンド | URL |
|-----|----------|-----|
| 補正・時系列（推奨） | `python 7_correction/dashboard/app.py` | http://127.0.0.1:8051/ |
| 旧統合 GUI | `python 8_dashboard/app.py` | http://127.0.0.1:8050/ |
...
```bash
# 9_legacy_v1/input_photos/CapturedFrames_*/ に JPG + synced GT
python run.py
python run.py --no-mediapipe   # 02 が既にある場合
python 8_dashboard/app.py     # http://127.0.0.1:8050/
```
