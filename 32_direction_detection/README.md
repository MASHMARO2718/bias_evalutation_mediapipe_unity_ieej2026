...
フレームレベルの符号付き方向角誤差。`40_calibration_framework` の符号付き補正に使用。
...
| ファイル | 場所 | 説明 |
|---------|------|------|
| MediaPipe CSV | `90_legacy_v1/mediapipe_processed/mediapipe_processed_csv/Y=*/` | フレーム別 3D 関節座標 |
...
- **入力元:** `90_legacy_v1/mediapipe_processed/` (MediaPipe CSV), `synced_joint_positions.csv` (GT)
- **出力先:** `40_calibration_framework/` が `detailed_results.csv` を使用
- **ダッシュボード:** `50_dashboard/` の可視化でも参照
